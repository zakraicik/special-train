...
if __name__ == "__main__":
    ...
    df = get_prices(polygon_client, start_date, end_date)
    ...
    while True:
        time.sleep(12)  ## 5 api calls per minute

        df = get_prices(polygon_client, start_timestamp + 300000, end_date)

        dfs.append(df)

        start_timestamp = df["timestamp"].max()

        if convert_millisecond_to_date(start_timestamp) >= end_date:
    ...
    aws_s3_client.put_object(
        ...
        Key=f"data/ethereum_prices_{start_date.strftime('%Y_%m_%d')}_{end_date.strftime('%Y_%m_%d')}.csv.gz",
